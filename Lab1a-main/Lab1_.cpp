...
public:
	...
	public:
		...
		bool Read_Medication_From_File(string filename = "Undef") {
		...
		bool Check_Admin_Login(string login = "Undef", string password = "Undef") {
			ifstream file;
			file.open("Admin.txt");
			string temp_log, temp_pas;
			if (file.is_open()) {
...
int main()
{
	...
	while (true) {
		...
		switch (input) {
		case 1:
			pharmacy.admin.Enter_As_Customer();
			break;
		case 2:
			pharmacy.admin.Enter_As_Admin();
			break;
		case 3: return 0;
		default:
			cout << "Error" << endl;
